@echo off
REM 필요한 패키지 설치 스크립트 (Windows용)
REM CUDA 11.8 환경에 최적화된 설치를 수행합니다.

echo ===== 텍스트 복원 프로젝트 설치 스크립트 (Windows) =====
...
REM 가상 환경 생성 (선택적)
set /p create_venv=가상 환경을 생성하시겠습니까? (y/n): 
if /i "%create_venv%"=="y" (
    echo 가상 환경 생성 중...
...
REM PyTorch 설치 (CUDA 11.8)
echo PyTorch 설치 중 (CUDA 11.8 지원)...
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118

REM Transformers 및 관련 라이브러리
